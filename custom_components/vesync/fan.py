...
PRESET_MODES = [FAN_MODE_AUTO, FAN_MODE_SLEEP, FAN_MODE_MANUAL]
...
def _async_setup_entities(devices, async_add_entities):
    ...
    for dev in devices:
        if DEV_TYPE_TO_HA.get(dev.device_type) == "fan":
            if dev.device_type == "Core200S":
                dev_list.append(VeSyncFanHA_Core200S(dev))
            else:
...
class VeSyncFanHA_Core200S(VeSyncDevice, FanEntity):
    ...
    def preset_modes(self):
        ...
        return [FAN_MODE_SLEEP, FAN_MODE_MANUAL]

    @property
    def preset_mode(self):
        """Get the current preset mode."""
        #_LOGGER.debug("*** preset_mode: %s", self.smartfan.mode)
        if self.smartfan.mode in (FAN_MODE_SLEEP, FAN_MODE_MANUAL):
    ...
    def extra_state_attributes(self):
        ...
        return {
            ...
            "display_state": self.smartfan.display_state,
            "child_lock": self.smartfan.child_lock,
            "night_light": self.smartfan.night_light,
    ...
    def set_preset_mode(self, preset_mode):
        ...
        if preset_mode == FAN_MODE_SLEEP:
            self.smartfan.sleep_mode()
        elif preset_mode == FAN_MODE_MANUAL:
            self.smartfan.manual_mode()
